...
//////	
////////============测试用==刷屏红绿蓝 ==============================
//	BTE_Solid_Fill(0,canvas_image_width,0,0,color16M_red,280,1424);
//	ENSleep(500);
//	BTE_Solid_Fill(0,canvas_image_width,0,0,color16M_green,280,1424);
//	ENSleep(500);
//	BTE_Solid_Fill(0,canvas_image_width,0,0,color16M_blue,280,1424);
//	ENSleep(500);

////==============验证显示区都在屏内============================
//	Active_Window_XY(0,0);
//	Active_Window_WH(LCD_width,LCD_legth);
//	Draw_Square(color16M_red,0,0,279,1423);
}

void PWM0_BackLight_ON(void)
{
		Set_PWM_Prescaler_1_to_256(3); //Time base is "Core_Freq / (Prescaler + 1)"//3
   	Select_PWM0_Clock_Divided_By_8();//8
  	Select_PWM0();
		Set_Timer0_Count_Buffer(10);  //設定階數
    Set_Timer0_Compare_Buffer(10);	//DUTY	 
    Start_PWM0();  
}



void PWM1_BackLight_ON(void)		  //10
{
	Set_PWM_Prescaler_1_to_256(4); //Time base is "Core_Freq / (Prescaler + 1)"
   	Select_PWM1_Clock_Divided_By_2();
  	Select_PWM1();
	Set_Timer1_Count_Buffer(10);  //設定階數
    Set_Timer1_Compare_Buffer(10);	//DUTY	 	   //10/2-1
    Start_PWM1();  
}
...
void RA8889_HW_Reset(void)
{
	...
	ENSleep(1); // XnRST have to keep low at least 256 OSC clocks. 1
	//delay_seconds(2);
...
void System_Check_Temp(void)
{
	...
	do
	{
		...
		if(system_ok==0 && i==5)
		{
			...
			ENDelayus(30); // XnRST have to keep low at least 256 OSC clocks. 
...
*/	
	...
	temp |= 0x01;
	LCD_DataWrite(temp);
	ENDelayus(100);			// it must wait 100us after Software_Reset.
...
/* 1: PLL enable; cannot change PLL parameter.*/
	...
	ENDelayus(100);// PLL lock time = 1024 T OSC clocks, if OSC=10MHz, PLL lock time = 100 us.  
...
void Set_LCD_Panel(void)
{
		...
		*/
			LCD_Horizontal_Non_Display(38);//INNOLUX800x600，46。
			LCD_HSYNC_Start_Position(210);//INNOLUX800x600，16~354。
			LCD_HSYNC_Pulse_Width(7);//INNOLUX800x600，1~40C
	...
	#ifdef MD070STN92	//##EDT_1024x600_MD070STN92
...
//			VSCAN_B_to_T();
		...
		//**[16h][17h]**//
			LCD_Horizontal_Non_Display(146);//EDT_1024x600!ABlank=146
		//**[18h]**//
			LCD_HSYNC_Start_Position(208);//EDT_1024x600!ABlank=210
		//**[19h]**//
			LCD_HSYNC_Pulse_Width(8);//EDT_1024x600!ABlank=30
		//**[1Ch][1Dh]**//
			LCD_Vertical_Non_Display(13);//EDT_1024x600!ABlank=18
		//**[1Eh]**//
			LCD_VSYNC_Start_Position(22);//EDT_1024x600!ABlank=22
		//**[1Fh]**//
			LCD_VSYNC_Pulse_Width(10);//EDT_1024x600!ABlank=5
...
//		DE_Low_Active();
		...
		LCD_HorizontalWidth_VerticalHeight(1920,720);//INNOLUX 800x480。
		LCD_Horizontal_Non_Display(42);//INNOLUX800x600，46。
		LCD_HSYNC_Start_Position(16);//INNOLUX800x600，16~354。
		LCD_HSYNC_Pulse_Width(30);//INNOLUX800x600，1~40。
		LCD_Vertical_Non_Display(32);//INNOLUX800x600，23。
		LCD_VSYNC_Start_Position(64);//INNOLUX800x600，1~147。
		LCD_VSYNC_Pulse_Width(7);//INNOLUX800x600，1~20。
...
//		DE_Low_Active();
		...
		LCD_HorizontalWidth_VerticalHeight(480,1920);//
		LCD_Horizontal_Non_Display(30);//
		LCD_HSYNC_Start_Position(30);//
		LCD_HSYNC_Pulse_Width(15);//
		LCD_Vertical_Non_Display(6);//
		LCD_VSYNC_Start_Position(6);//
		LCD_VSYNC_Pulse_Width(18);//
...
//		DE_Low_Active();
		...
		LCD_HorizontalWidth_VerticalHeight(280, 1424);//
		LCD_Horizontal_Non_Display(30);//
		LCD_HSYNC_Start_Position(40);//
		LCD_HSYNC_Pulse_Width(30);//
		LCD_Vertical_Non_Display(12);//
		LCD_VSYNC_Start_Position(15);//
		LCD_VSYNC_Pulse_Width(3);//
