...
//	 IDEC_Source_Start_Address(addr);    //JPG起cl|i?}
...
)
{
	...
	SPI_Master_FIFO_Data_Put(dma_page_addr);   //page address LSB
	nSS_Inactive();
	ENDelayus(500); //delay for buffer load
  ...
  BTE_Destination_Image_Width(canvas_image_width);
...
void copyLayer2BufferDataToLayer1DisplayArea(void)
{
  BTE_S0_Memory_Start_Address(Layer2);
  BTE_S0_Image_Width(canvas_image_width);//Image_Width = canvas_image_width
  BTE_S0_Window_Start_XY(0,0);

  BTE_Destination_Memory_Start_Address(Layer1);
  BTE_Destination_Image_Width(canvas_image_width);	//Image_Width = canvas_image_width
  BTE_Destination_Window_Start_XY(0,0);	
	
  BTE_Window_Size(LCD_width,LCD_legth); 
