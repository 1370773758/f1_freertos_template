...
#define LCD_XSIZE       1424		//旋转后
#define LCD_YSIZE       280
...
static unsigned int _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  unsigned int PixelIndex;
    ...
    GUI_USE_PARA(y);
    {
      ...
      //
	  s_ucRA8875BusyNow = 1;
    putPixel(xPhys, yPhys,PixelIndex);//竖屏横用 横屏竖用 add jackey
 	  // LCD_GetPixel(xPhys, yPhys);
 	  s_ucRA8875BusyNow = 0;
    }
    #if (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
      #undef xPhys
...
//  LCD_PIXELINDEX ColorIndex;
  if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR) {
    for (; x0 <= x1; x0++) {
	  ...
	  #if emWin_Optimize
		s_ucRA8875BusyNow = 1;
		LCD_DrawLineH(LOG2PHYS_X(x0,y), LOG2PHYS_Y(x0,y), LOG2PHYS_Y(x1,y), LCD_COLORINDEX);
		s_ucRA8875BusyNow = 0;
	  #else
    //_FillRect(pDevice, LOG2PHYS_X(x0,y), LOG2PHYS_Y(x0,y), LOG2PHYS_X(x1,y),LOG2PHYS_Y(x0,y));
		LCD_PIXELINDEX ColorIndex;
		ColorIndex = LCD_COLORINDEX;//LCD__GetColorIndex();
		for (; x0 <= x1; x0++) {
...
//  LCD_PIXELINDEX ColorIndex;
  if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR) {
    ...
    #if emWin_Optimize
		s_ucRA8875BusyNow = 1;
		LCD_DrawLineV(LOG2PHYS_X(x,y0), LOG2PHYS_Y(x,y0), LOG2PHYS_X(x,y1), LCD_COLORINDEX);
		s_ucRA8875BusyNow = 0;
	#else
		LCD_PIXELINDEX ColorIndex;
		ColorIndex = LCD_COLORINDEX;//LCD__GetColorIndex();
...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
	...
	LCD_DrawRect(LOG2PHYS_X(x0,y0), LOG2PHYS_Y(x0,y0), LOG2PHYS_X(x1,y1), LOG2PHYS_Y(x1,y1), LCD_COLORINDEX);
...
#if emWin_Optimize
	s_ucRA8875BusyNow = 1;
	LCD_DrawHColorLine(LOG2PHYS_X(x, y),LOG2PHYS_Y(x, y), xsize, (uint16_t *)p);
	s_ucRA8875BusyNow = 0;	
#else
	for (;xsize > 0; xsize--, x++, p++) 
	{
